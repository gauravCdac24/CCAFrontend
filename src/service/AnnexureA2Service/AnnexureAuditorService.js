...
const getAnnualAuditPeriodDetails = (obj) =>{
...
const addEKYCAcMonthDetails = (obj)=>{
	...
	const ekycMonthDetails = obj1.map((row) => ({
		...row,
		eKYCAcMonthId: row.eKYCAcMonthId ? encrypt(row.eKYCAcMonthId) : '',
	}));
...
const addAccountBasedDetails = (obj)=>{
	...
	if(obj1.ekycAcMainId)
		obj1.ekycAcMainId = encrypt(obj1.ekycAcMainId)
...
const addCryptoTokenDetails = (obj)=>{
	...
	obj1.forEach((element, index) => {
		...
		//const formattedDate = new Date(element.fipCertUpTo).toISOString(); 
		...
		if(element?.secAuditDetails){
			formData.append(`cryptoTokenDetails[${index}].secAuditDetails`, element.secAuditDetails)
		}	
		formData.append(`cryptoTokenDetails[${index}].cryptoTokDetailsId`, element.cryptoTokDetailsId ? encrypt(element.cryptoTokDetailsId) : '')
...
const addASPDetails = (obj) => {
	...
	obj1.forEach((element, index) => {
		formData.append(`aspsAuditOverdueCountFileName`, element.aspsAuditOverdueCountFileName)
		formData.append(`aspCountFileName`, element.aspCountFileName)
		if(element?.aspCount){
			formData.append(`aspCount`, element.aspCount)
		}
		if(element?.aspsAuditOverdueCount){
			formData.append(`aspsAuditOverdueCount`, element.aspsAuditOverdueCount)
		}
		formData.append(`aspDetailsId`, element.aspDetailsId ? encrypt(element.aspDetailsId) : '')
...
const addPublicInfoDetails = (obj)=>{
	...
	const publicInfoDetails = obj1.map((row) => ({
		...row,
		caWebDetailsId: row.publicInfoDetailsId ? encrypt(row.publicInfoDetailsId) : '',
	}));
...
const addAuditorVerification = (obj)=>{
	...
	obj1.apiNumber = encrypt(obj1.apiNumber)
	obj1.auditorVerification= encrypt(obj1.auditorVerification)
	return API.post(AUDIT_SERVICE+ADD_DOWNTIME_DETAILS, obj1);
}
