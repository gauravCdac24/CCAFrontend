...
import { Box, Grid, Button, Checkbox, Typography, FormControlLabel, Link } from '@mui/material';
...
const EditLicenseCessationChecklist = () => {
  const { id } = useParams();
...
console.log("getAllActiveDataForCCAOfficer-=-=-=-=-=-=->",response.data);
  ...
  const handleSubmit = () => {
    ...
    CessationService.editAllData(formData)
      .then((response) => {
        showAlert({
  ...
  return (
    ...
    <FormWrapper headingText="Edit License Cessation Checklist">
                <Box component="form" noValidate sx={{ mt: 2, p: 2 }}>
                <Grid container spacing={2}>
  {Object.keys(checklist).map((key) => {
    const item = checklist[key];
    const cessationDocuments = getAllActiveDataForCCAOfficer?.cessationDocument || [];
    console.log("cessationDocuments-=-=-=-=-=->", cessationDocuments);

    const relatedUndertaking = cessationDocuments.filter(
    ...
    const isDisabled = relatedUndertaking.length === 0;

    const relatedUndertakings = getAllCessationAppUndertakings.filter(
      ...
      <Grid item xs={12} key={item.id}>
        <Box display="flex" alignItems="center" justifyContent="space-between" flexWrap="wrap">
          <FormControlLabel
            control={
              <Checkbox
                checked={isDisabled || !!item.checked}
          ...
          >
            Attach File
            <input type="file" hidden onChange={handleFileUpload(key)} />
          </Button>
          {item.file && (
          ...
          {item.error && (
            <Typography variant="body2" color="red" sx={{ marginRight: '16px' }}>
              {item.error}
            </Typography>
          )}
          <Box>
            {relatedUndertakings.length > 0 ? (
              <Link
                href="#"
                underline="hover"
                onClick={() => handleDownloadClick(relatedUndertakings[0].appUndertakingId)}
              >
                Download Document
              </Link>
            ) : (
...
export default EditLicenseCessationChecklist;
