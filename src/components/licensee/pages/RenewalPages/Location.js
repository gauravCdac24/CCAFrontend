...
import { Box, Grid, TextField, FormControl, InputLabel, Select, Button, MenuItem, Typography, IconButton, FormControlLabel, Radio, RadioGroup, FormHelperText, Tooltip, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, DialogTitle, DialogContent, DialogActions ,Dialog, Checkbox, styled} from '@mui/material';
...
const errorMsg = {
    ...
    blockNo1: {
        ...
        length: "The length of Block No (Office) must be between 1 and 10 characters.",
    },
    village1: {
        blank: "Village (Office) cannot be empty.",
        length: "The length of Village (Office) must be between 1 and 50 characters.",
    },
    postOffice1: {
        blank: "Post Office (Office) cannot be empty.",
        length: "The length of Post Office (Office) must be between 1 and 50 characters.",
    },
    subDivision1: {
        blank: "Subdivision (Office) cannot be empty.",
        length: "The length of Subdivision (Office) must be between 1 and 50 characters.",
    },
    country1: {
    ...
    blockNo2: {
        ...
        length: "The length of Block No (Communication) must be between 1 and 10 characters.",
    },
    village2: {
        blank: "Village (Communication) cannot be empty.",
        length: "The length of Village (Communication) must be between 1 and 50 characters.",
    },
    postOffice2: {
        blank: "Post Office (Communication) cannot be empty.",
        length: "The length of Post Office (Communication) must be between 1 and 50 characters.",
    },
    subDivision2: {
        blank: "Subdivision (Communication) cannot be empty.",
        length: "The length of Subdivision (Communication) must be between 1 and 50 characters.",
    },
    country2: {
    ...
    city2: {
        ...
        length: "The length of City (Communication) must be between 1 and 50 characters.",
    },
    pin2: {
...
const [fileName1, setFileName1] = useState('');
    ...
    const processApplicationData = (e) => {
        ApplicationForm.getApplicationForm2ByUsername(userName)
            ...
            const updatedDisabledLocationTypes = locationDetails.map(detail => detail.locationType);
            console.log("Updated Disabled Location Types:", updatedDisabledLocationTypes); // Debugging

        // Update selected location types, including "Registered Office"
        setSelectedLocationTypes(prevTypes => [...prevTypes, "Registered Office"]);

        setDisabledLocationTypes(prevTypes => [...prevTypes,...updatedDisabledLocationTypes]);
...
// }, [userName, states, cities]);
    ...
    const locationTypeOptions = [
        { locationTypeName: 'PR', isMandatory: true,fullName:'Primary Site Details' },
        { locationTypeName: 'DR', isMandatory: true ,fullName:'Disaster Recovery Site Details' },
        { locationTypeName: 'Registered Office', isMandatory: false ,fullName:'Registered Office Site Details' },
        { locationTypeName: 'Other' , isMandatory: false,fullName:'Other Site Details' },
    ];
            ...
            ApplicationForm.getApplicationForm5ByUsername(userName)
                .then((response) => {
                    ...
                    const filteredLocationMap = mappedLocationMap.filter((location) =>
                        locationTypeOptions.some(option => option.locationTypeName.trim() === location.locationType.trim())
                    );
    
                    console.log('Filtered Location Map:', filteredLocationMap);
                    // Combine mapped address details with location types
                    const combinedLocationDetails = addressDTOList.map((address, index) => {
                        ...
                        return {
                            ...address,
                            locationType: (matchingLocation && matchingLocation.locationType) || "Unknown",
                            addressId: (matchingLocation && matchingLocation.addressId) || "Unknown",
                            locationId: (matchingLocation && matchingLocation.locationId) || "Unknown",
                            intentAppId: (matchingLocation && matchingLocation.intentAppId.intentAppId) || "Unknown",
                            userName: (matchingLocation && matchingLocation.userName) || userName, // Ensure userName is included
                            certificateName: (matchingLocation && matchingLocation.certificateName) || null,
                            certificationLevel: (matchingLocation && matchingLocation.certificationLevel) || "Unknown",
                            companyName: (matchingLocation && matchingLocation.companyName) || "Unknown",
                        };
                    });
                    ...
                    const updatedDisabledLocationTypes = filteredLocationMap.map(detail => detail.locationType);
    
                    // Update the selected location types (if needed)
                    setSelectedLocationTypes(prevTypes => [...prevTypes, communicationAddressFormValues.locationType]);
   ...
   const handleInputChange = (event) => {
    ...
    const validateForm = () => {
        ...
        } else if (communicationAddressFormValues.blockNo.length < 3 || communicationAddressFormValues.blockNo.length > 20) {
        ...
        } else if (communicationAddressFormValues.village.length < 3 || communicationAddressFormValues.village.length > 50) {
        ...
        } else if (communicationAddressFormValues.postOffice.length < 3 || communicationAddressFormValues.postOffice.length > 50) {
        ...
        } else if (communicationAddressFormValues.subDivision.length < 3 || communicationAddressFormValues.subDivision.length > 50) {
    ...
    const handleAddLocation = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            const newLocation = {
                ...
                certificationDocument: communicationAddressFormValues.certificationDocument,  // Persist certificationDocument
            };
            ...
            setSelectedLocationTypes(prevTypes => [...prevTypes, communicationAddressFormValues.locationType]);
            setDisabledLocationTypes([...disabledLocationTypes, communicationAddressFormValues.locationType]);
        } else {
  ...
  const [selectedOption, setSelectedOption] = useState();
    ...
    const EditComponent = ({formValues, stateList, cityList}) =>{

        setEditFormValues(()=>formValues);
        setFilteredCitie(cityList);
        ...
        <TextField
            label="Location Type"
            name="locationType"
            value={editFormValues.locationType||''}
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12} sm={6}>
                ...
                <TextField
                    ...
                    variant='outlined'
                    onChange={handleEditFormChange}
                    value={editFormValues.postOffice||''}
                    size="small"
                    inputProps={{ maxLength: 30 }}
                />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
                ...
                <TextField
                    ...
                    variant='outlined'
                    onChange={handleEditFormChange}
                    value={editFormValues.subDivision||''}
                    size="small"
                    inputProps={{ maxLength: 30 }}
                />
            </Grid>
    ...
    const handleEditClick = (index) => {
        ...
        const fvalues = { ...communicationAddressFormValues.LocationDetails[index] };
        const filteredStates = states.filter(state => state.countryId.countryId === parseInt(editFormValues.country)); 
        const filteredCities = cities.filter(city => city.stateId.stateId === parseInt(editFormValues.state));
        
        
           
            showAlert({
                messageTitle: 'Edit Address',
                messageContent: <EditComponent  formValues={fvalues} stateList={filteredStates} cityList={filteredCities} />,
                confirmText: 'Update',
    ...
    const handleUpdate = () => {
        ...
        setCommunicationAddressFormValues(prevValues => {
            ...
            updatedList[editIndex] = editFormValues;  // Apply the changes
            return {
                ...prevValues,
    ...
    // };
     ...
     const handleDeleteAddress = (index) => {
        const updatedLocationDetails = communicationAddressFormValues.LocationDetails.filter((_, i) => i !== index);
        const deletedLocationType = communicationAddressFormValues.LocationDetails[index].locationType;
        ...
        setDisabledLocationTypes(disabledLocationTypes.filter(type => type !== deletedLocationType));
    };
...
const [selectedLocationFullName, setSelectedLocationFullName] = useState('');
    const handleLocationTypeChange = (event) => {
        ...
        const selectedOption = locationTypeOptions.find(option => option.locationTypeName === value);
   ...
   const locationFullName= locationTypeOptions.find(option => (option.locationTypeName===locationType));
    ...
    const handleFormSubmit = (e) => {
        ...
        const mandatoryTypes = locationTypeOptions.filter(type => type.isMandatory);
        ...
        if (missingMandatory) {
           ...
           // alert('Please select all mandatory location types before proceeding.');
            ...
            showAlert({
                messageTitle: 'Alert',
                messageContent: "Please select all mandatory location types before proceeding.",
                confirmText: 'Ok',
            });
        } else {
            console.log(JSON.stringify( communicationAddressFormValues.LocationDetails))
            ...
            const formData = new FormData();
                    ...
                    communicationAddressFormValues.LocationDetails.forEach((item, index) => {
                        formData.append(`locationDetails[${index}].locationType`, item.locationType || '');
                        formData.append(`locationDetails[${index}].blockNo`, item.blockNo || '');
                        formData.append(`locationDetails[${index}].village`, item.village || '');
                        formData.append(`locationDetails[${index}].postOffice`, item.postOffice || '');
                        formData.append(`locationDetails[${index}].subDivision`, item.subDivision || '');
                        formData.append(`locationDetails[${index}].country`, item.country || '');
                        ...
                        formData.append(`locationDetails[${index}].intentAppId`, item.intentAppId || '');
                        formData.append(`locationDetails[${index}].addressId`, item.addressId || '');
                        formData.append(`locationDetails[${index}].locationId`, item.locationId || '');
                        formData.append(`locationDetails[${index}].certificationLevel`, item.certificationLevel || '');
                        formData.append(`locationDetails[${index}].companyName`, item.companyName || '');
                        if(item?.certificationDocument){
                        formData.append(`locationDetails[${index}].certificationDocument`, item.certificationDocument || '');
                        }
                        formData.append(`locationDetails[${index}].certificateName`, item.certificationName || '');
                    });
            ...
            ApplicationForm.addNewApplicationForm5 (formData)
                .then((response) => {
                    ...
                    showAlert({
                        ...
                        confirmText: 'Ok',
                        onConfirm: () => { handleNext(); }  // Handle next step navigation
                    });
                })
                .catch((err) => {
            ...
            <Grid item xs={12} sm={3}>
                ...
                <FormControl variant="outlined" size="small" sx={{ width: '100%', mt: 1 }}>
                    ...
                    >
                        ...
                        <MenuItem  key={option.locationTypeName}
                        value={option.locationTypeName}  disabled={disabledLocationTypes.includes(option.locationTypeName)}>  {option.locationTypeName} {option.isMandatory ? '(Mandatory)' : ''}</MenuItem>
                    ))}
                    </Select>
                    {formErrors.locationType && <FormHelperText sx={{ ml: 0 }} error>{formErrors.locationType}</FormHelperText>}
                </FormControl>
            </Grid>
        </Grid>
        {locationType && (
  ...
  <Box display="flex" alignItems="center">
    <Typography variant="body1">Have you own Data Center(DC)?</Typography>
    <RadioGroup
      row
      ...
      <FormControlLabel value="no" control={<Radio />} label="No" />
        ...
        {knownByOtherName && (
            <>
                     <Grid item xs={12} sm={6} >
                        <InputLabel shrink={false} htmlFor="blockNo">
                            <Typography variant='h5' sx={{ fontWeight: 'bold', textAlign: 'center' }}>{selectedLocationFullName}</Typography>
                        </InputLabel>
                        </Grid>
  ...
  <Grid item xs={12} sm={6}>
    <InputLabel shrink={false} htmlFor="pdffile">
      <Typography variant="body1">
        Upload Certificate Document (Only PDF and Max allowed size 5 MB)
      </Typography>
    </InputLabel>
...
<TableContainer sx={{ mt: 3 }}>
  <Table>
    ...
    <TableBody>
      {communicationAddressFormValues.LocationDetails.map((location, index) => (
        <TableRow key={index}>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{location.locationType}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{location.blockNo}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{location.village}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{location.postOffice}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{location.subDivision}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{getCountryNameById(parseInt(location.country))}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{getStateNameById(parseInt(location.state))}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{getCityNameById(parseInt(location.city))}</TableCell>
          <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>{location.pin}</TableCell>
