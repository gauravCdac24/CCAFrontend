...
import { changeColorMode, changeFooterColor, changeMode, changeSidebarColor, changeAppbarColor } from "../../../store/ColorMode/Reducer";
...
const AccessibilityUser = () =>{
  ...
  const SidebarColorButton = ({color}) =>{
    return(
          ...
          onClick={()=>dispatch(changeSidebarColor(color))}
          >
  ...
  const drawer = (
    ...
    <Box component="fieldset" sx={{
      ...
      <legend><Typography sx={{fontSize: '14px', backgroundColor: "#000", color: "#FFF", padding: "4px", borderRadius: "10px"}}>Sidebar</Typography></legend>
      ...
      <Grid container spacing={1} sx={{
        ...
        {sidebarColors.map((color, index)=>(
          <Grid item key={index}>
            <SidebarColorButton color={color} />
          </Grid>
...
export default AccessibilityUser;
