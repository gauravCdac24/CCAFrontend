...
let isBolean=false;
    ...
    const handleSubmit = () => {
      if(2<=getAllActiveDataForCCAOfficer.length){
        isBolean=true;
      }
      const formData = new FormData();
      formData.append("isBolean",isBolean)
...
console.log("formData=-=-=-=-=-=-=-=->",formData)
      ...
      CessationService.rejectApplicationForCCAOfficer(formData)
        .then((response) => {
          showAlert({
...
console.log("formData=-=-=-=-=-=-=-=->",formData)
      ...
      CessationService.approveApplicationForCCAOfficer(formData)
        .then((response) => {
          showAlert({
            messageTitle: 'Success',
            messageContent: response.data,
...
<Grid container sx={{mt : 2}}>
  ...
  <Grid container direction="row" sx={{ mt: 4 }} spacing={2} justifyContent="center" alignItems="center">
    ...
    <Grid item>
      ...
      >
        {(2 <= getAllActiveDataForCCAOfficer.length) ? 'Recommand for Rejection' : 'Submit for Review'}
      </Button>
    </Grid>
  </Grid>
</Grid>
