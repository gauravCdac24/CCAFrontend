...
const AuditorDetails = (() => {
   ...
   const getAllAuuditorByUserName = () => {
    ...
    AnnualAuditService.getAuditorApplicationbyApplicatantUserName(ApplicantUserName)
        .then((response) => {
            ...
            const updatedValues = {
                ...audiAgencyFormValues,
                auditAgencyName: response.data.auditAgencyName ?? audiAgencyFormValues.auditAgencyName,
                applicantUserName: response.data.applicantUserName ?? audiAgencyFormValues.applicantUserName,
                intentAppId: response.data.intentAppId ?? audiAgencyFormValues.intentAppId,
                undertakingFileName: response.data.undertakingFileName ?? audiAgencyFormValues.undertakingFileName,
                undertakingFile: response.data.undertakingFile ?? audiAgencyFormValues.undertakingFile,
                auditorDesscription: response.data.auditorDesscription
                    ? response.data.auditorDesscription?.map((item) => ({
                        ...
                        certificateExpiry: processDates(item.certificateExpiry),
                    }))
                    : audiAgencyFormValues.auditorDesscription,
                auditScope: response.data.auditScope
                    ? response.data.auditScope.map((item) => ({
                        ...
                        startDate: processDates(item.startDate),
                        endDate: processDates(item.endDate),
                    }))
                    : audiAgencyFormValues.auditScope,
            };

            setAudiAgencyFormValues(updatedValues);

            // Explicitly set fileName and log auditor names
            setFileName(response.data.undertakingFileName || '');
            if (response.data.auditorDesscription) {
                response.data.auditorDesscription.forEach((auditor) => {
                    console.log('Auditor Name:', auditor.auditorName);
                });
            }
        })
        .catch(() => {
...
audiAgencyFormValues.auditScope.forEach((item, index) => {
    ...
    const downloadfile = async (auditorDescriptionId, documentTitle) => {
        try {
            ...
            const response = await AnnualAuditService.viewFile(auditorDescriptionId);
    ...
    const handleRejectedSubmit = (e) => {
      ...
      const approvedFunction = () => {
        ...
        formData.append(`remark`, audiAgencyFormValues.remark);
...
AnnualAuditService.approvedAuditForm(formData)
            .then((response) =>
                showAlert({
                    messageTitle: 'Success',
                    messageContent: response.data,
                    confirmText: 'Ok',
                    onConfirm: () => window.location.href = '/cca/uploadannualundertakings',
                })
            )
            .catch((err) => {
                // Handle error response
...
audiAgencyFormValues.auditScope.forEach((item, index) => {
    ...
    formData.append(`auditScope[${index}].endDate`, item.endDate);
        ...
        AnnualAuditService.rejectedAuditForm(formData)
            .then((response) =>
                showAlert({
                    messageTitle: 'Success',
                    messageContent: response.data,
    ...
    const handleChanges = (e, fieldName) => {
        ...
        // Reset error for the current field if valid
        setErrors((prevErrors) => ({
            ...prevErrors,
            [fieldName]: "",
        }));
...
>
    ...
    {Array.isArray(audiAgencyFormValues?.auditors) &&
        audiAgencyFormValues?.auditors.map((auditor, auditorIndex) => {
            const fullName = `${auditor.firstName || ''} ${auditor.lastName || ''}`.trim();
            console.log(`MenuItem Rendered: ${fullName}`);
            return (
                <MenuItem key={auditorIndex} value={fullName}>
                    {fullName || 'Unnamed Auditor'}
                </MenuItem>
            );
        })}
</Select>
