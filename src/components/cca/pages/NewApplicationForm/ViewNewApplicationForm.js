      ...
      console.log("abc=-->",uname)
              ...
              if (hasMatchingUsername === '') {
                    ...
                    const matchedType = allApplicationTypeList.find(
                      (type) => String(type.appTypeMasterId) === String(result[i]?.applicationType)
                    );
                    const applicationType = matchedType ? matchedType?.appType : 
...
console.log("applicationCounts=======>",JSON.stringify(applicationCounts))
 ...
 const routeRootPath = useSelector((state) => state.jwtAuthentication.rolePath);
  ...
  const renderIndividualViewData = (applicantUserName) => {
    ...
    navigate(`${routeRootPath}/rejectedreviewapplication/indivreviewapplication/${userName}`)
        ...
        const renderFirmViewData = async(applicantUserName) => {
            ...
            const userName=encodeURIComponent (encrypt(applicantUserName))
            navigate(`${routeRootPath}/rejectedreviewapplication/firmreviewapplication/${userName}`)
        ...
        const renderGovernmentViewData = async(applicantUserName) => {
            ...
            const userName=encodeURIComponent (encrypt(applicantUserName))
            navigate(`${routeRootPath}/rejectedreviewapplication/govreviewapplication/${userName}`)
    ...
    const columns = [
        ...
        // { field: 'updated', headerName: 'Updated', resizable: false, width: 150 },
        {
            ...
            renderCell: (params) => {
                ...
                } else if (params.row.applicationCurrentStatus === 'Recommended Reject For Review') {
                    return (
                        <Tooltip title="Edit">
                            <GridActionsCellItem 
