...
const LicenseeDashboard = () =>{
  ...
  const getStateList = async() => {
    ...
    try {
     ...
     const getInfoData = () => {
      setIsLoading(true);
      DashboardService.getLicenseeDashboardDetails()
      .then((response)=>{
    ...
    const getGraphDataEsignDSC = async () => {
      ...
      const obj = {
        selectedCA: btoa(decodeURIComponent(encodeURIComponent(username))),
      ...
      console.log("[LicenseeDashboard] getGraphDataEsignDSC called with:", {
        username,
        selectedMonth,
        selectedState,
        selectedYear,
      ...
      try{
          ...
          console.log("[LicenseeDashboard] API response:", response.data);
          setGraphData(response.data);
        }
      catch(err){
        console.error("[LicenseeDashboard] API error:", err);
      }
    };
    

    const navigateToASP = () => {
      navigate(`${rolePath}/licenseedashboard/lasplist`);
    }
  
    const navigateToDSCIssued = () => {
      navigate(`${rolePath}/licenseedashboard/ldscissued`);
    }

    const navigateToeSignIssued = () => {
      navigate(`${rolePath}/licenseedashboard/lesignissued`);
    }
    ...
    useEffect(()=>{
      if (selectedMonth.length === 0 || selectedState.length === 0 || selectedYear.length === 0) return;
      getGraphDataEsignDSC();
    },[selectedMonth, selectedState, selectedYear])
          ...
          <Grid container spacing={1} >
                  ...
                  <Grid>
                    <Grid container direction="column">
                      <Grid  sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>Total ASP</Grid>
                      ...
                      {/* <AnimatedNumbers
                        ...
                        animateToNumber={infoList?.totalASP || 0}
  ...
  <Grid container spacing={1} sx={{color: '#000000'}}>
    <Grid    size={{ xs: 12, sm: 12, md: 'grow' }} sx={{boxShadow: 'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px', m: 1, borderRadius: '4px'}}>
          ...
          <CustomBarChart series={[{
            data: [infoList.totalASP, infoList.totalDSCIssued, infoList.totaleSignIssued]
          }]} 
                    categories={['Total ASP', 'Total DSCs Issued', 'Total eSign Issued']}  
                    height={400} 
                    id="licenseebar1"
                    distributed={true}
                    />
    ...
    <Grid size={{ xs: 12, sm: 12, md: 'grow' }} sx={{boxShadow: 'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px', m: 1, borderRadius: '4px'}}> 
          ...
          <CustomPieChart 
              series={[infoList.totalASP, infoList.totalDSCIssued, infoList.totaleSignIssued]} 
              categories={['Total ASP', 'Total DSCs Issued', 'Total eSign Issued']} 
              height={400} 
              id="licenseepie2"
              legend={true}
...

  {
                ...
                <Grid container >
                    ...
                    <Box >
                        ...
                        <Box sx={{color: "#000000"}}>
                          <CustomAreaChart series={[...graphData.yearwise]} categories={[...graphData.years]}   id="licenseed1" legend={true}/>
                          {/* <CustomAreaBrushChart series={[...graphData.yearwise]} categories={[...graphData.years]}   id="licenseed1" /> */}
                        </Box>
                    </Box>
                ...
                <Grid container >
                    ...
                    <Box >
                        ...
                        <Box sx={{color: "#000000"}}>
                            <CustomAreaChart series={[...graphData.statewise]} categories={[...graphData.states]} colors={['#008B8B', '#F44336']} id="licenseed4" legend={true}/>
                            {/* <CustomAreaBrushChart series={[...graphData.statewise]} categories={[...graphData.states]} colors={['#008B8B', '#F44336']} id="licenseed4"/> */}
                        </Box>
                    </Box>     
                    </Grid>
                ...
                {graphData?.espEkycModeApproved.length>0 && (
                <Grid container >
                    ...
                    <Grid size={{ xs: 12, sm: 12, md: "grow" }} sx={{m: 1, boxShadow: 'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px', borderRadius: '4px'}}>
                    <Box >
                        <Box sx={{fontWeight: '600', m: 1}}>Approval of eKYC modes</Box>
                        <Box sx={{color: "#000000", height: 'auto'}}>     
