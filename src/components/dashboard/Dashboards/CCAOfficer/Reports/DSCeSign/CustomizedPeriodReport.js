...
const CustomizedPeriodReport = () => {
    ...
    const getCustomizedPeriodReportData = async () => {

        try{
            setIsLoading(true);
            const response = await DashboardService.getCustomizedPeriodReportOnDSCEsignData(selectedMonth, selectedYear, selectedMonth1, selectedYear1);
    ...
    const downloadCustomizedPeriodDSCEsignDataReport = async() => {


         try {
                    setIsLoading(true);
                    const response = await DashboardService.downloadCustomizedPeriodDSCEsignDataReport(selectedMonth, selectedYear, selectedMonth1, selectedYear1);
                    console.log(response);
                    const blob = new Blob([response.data], { type: response.headers['content-type'] });
                    const link = document.createElement('a');
                    link.href = window.URL.createObjectURL(blob);
                    const contentDisposition = response.headers['content-disposition'];
                    const filename = "Customized Period Report";
    ...
    return(
                ...
                <Grid  size={{ xs: 12, sm: 12, md: 'grow' }}>
                    ...
                    <Grid  size={{ xs: 12, sm: 12, md: 'grow' }}>
                        ...
                        <Grid  size="grow" sx={{mt: 4}}>
                            ...
                            <Button variant="contained"  onClick={downloadCustomizedPeriodDSCEsignDataReport}>
        ...
        <CustomTable customTitle = "Customized Period Report of eSign & DSC" columns={columns} rows={data}  hideColumnsForExport={['']} pageSizeOptions={[10, 25, 50, 100]} />
...
export default CustomizedPeriodReport;