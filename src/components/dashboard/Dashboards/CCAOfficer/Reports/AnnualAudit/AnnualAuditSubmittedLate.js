...
const AnnualAuditSubmittedLate = () => {
    ...
    const navigate = useNavigate();
     ...
     const getAnnualAuditSubmittedLate = async () => {
    
            try{
                setIsLoading(true);
                const response = await DashboardService.getAnnualAuditSubmittedLate();
    ...
    const downloadAnnualAuditSubmittedLate = async() => {


         try {
                    setIsLoading(true);
                    const response = await DashboardService.downloadAnnualAuditSubmittedLate();
                    console.log(response);
                    const blob = new Blob([response.data], { type: response.headers['content-type'] });
                    const link = document.createElement('a');
                    link.href = window.URL.createObjectURL(blob);
                    const contentDisposition = response.headers['content-disposition'];
                    const filename = "AnnualAuditSubmittedLate";
    ...
    return(
            ...
            <Box component="div">
                ...
                <Grid container spacing={2} direction={'column'}>
                    ...
                    <Grid  sx={{display: 'flex', justifyContent:'right', alignItems:'right', mr: 2}}>
                        <Button variant="contained"  sx={{mr: 2}} onClick={downloadAnnualAuditSubmittedLate}>
        ...
        <CustomTable customTitle = "Report on Annual Audits Submitted Late" columns={columns} rows={data}  hideColumnsForExport={['']} pageSizeOptions={[10, 25, 50, 100]} />
...
export default AnnualAuditSubmittedLate;