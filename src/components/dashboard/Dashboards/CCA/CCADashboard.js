...
const CCADashboard = () =>{
  ...
  const getStateList = async() => {
    ...
    const getGraphDataEsignDSC = async () => {
      ...
      console.log("[CCADashboard] getGraphDataEsignDSC called with:", {
        selectedCA,
        selectedMonth,
        selectedState,
        selectedYear,
      ...
      try{
          ...
          console.log("[CCADashboard] API response:", response.data);
          setGraphData(response.data);
        }
      catch(err){
        console.error("[CCADashboard] API error:", err);
      }
    };
    ...
    useEffect(()=>{
      if (selectedCA.length === 0 || selectedMonth.length === 0 || selectedState.length === 0 || selectedYear.length === 0) return;
    ...
    const navigateToRegisteredCA = () => {
      navigate(`${rolePath}/ccadashboard/registeredca`);
    }
    const navigateToRegisteredESP = () => {
      navigate(`${rolePath}/ccadashboard/registeredesp`);
    }
    const navigateToDSCIssued = () => {
      navigate(`${rolePath}/ccadashboard/dscissued`);
    }
    const navigateToeSignIssued = () => {
      navigate(`${rolePath}/ccadashboard/esignissued`);
    }
