...
const AuditorControls = () => {
  ...
  const[remarks,setRemarks]=useState([])
   ...
   const getAllRemarksAuditControl = () => {
    setLoading(true);
    AnnualAuditService.viewRemarks(ApplicantUserName)
      .then((response) => {
...
const downloadfile = async ( documentTitle) => {
  try {
      ...
      const response = await AnnualAuditService.viewDocumentName(documentTitle);
...
const getAllAuditNCReport = () => {
  setLoading(true);
  AnnualAuditService.auditNCReport(ApplicantUserName)
    .then((response) => {
...
const getAllShortComming= () => {
    setLoading(true);
    AnnualAuditService.getAllShortComming(ApplicantUserName)
        .then((response) => {
...
console.log("allShortComming.length=====>",allShortComming.length)
console.log("allMinimumAttemptList.applicationAudit=====>",applicationReview)
...
const handleConfrimSubmit = () => {
  ...
  AnnualAuditService.changeTheStatusRejected(applicantRemarks)
  .then((response) => {
    //console.log(response.data)
      showAlert({
          messageTitle: 'Success',
...
const handleSubmit = () => {
  ...
  applicantRemarks.append("remarks",radioValue.remarks);
  applicantRemarks.append("applicantUserName",radioValue.applicantUserName);
  AnnualAuditService.changeTheStatusApprove(applicantRemarks)
  .then((response) => {
    //console.log(response.data)
      showAlert({
          messageTitle: 'Success',
                          ...
                          <TableBody>
                            {subCriteria.auditParameterPayload?.map((parameter, paramIndex) => {
                              ...
                              <React.Fragment key={`parameter-${paramIndex}`}>
                                <TableRow>
                                  <TableCell colSpan={8} sx={{ border: 0.5, fontWeight: "bold" }}>
  ...
  />
                                    ...
                                    <TableCell sx={{ border: 0.5 }}>
                                      <input type="file" onChange={(event) => handleFileChange(event, controlIndex)}  disabled/>
        ...
        <span>{fileName[controlIndex]}</span>
                                    ...
                                    <TableCell sx={{ border: 0.5 }}>
                                      <Button
                                        ...
                                        disabled // Disable if file is not selected for this row
                                      >
                                        Save
                                      </Button>
                                    </TableCell>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              ...
              <TextField
                ...
                rows={4}
                variant="outlined"
                value={remarks.applicantRemarks	}
                disabled
                fullWidth
    ...
    >
      <span>{auditNCReport.ncs}</span>
          ...
          <Grid container direction="row" sx={{ mt: 4 }} spacing={2} justifyContent="center" alignItems="center">
                        ...
                        <Grid item  >
                            <Button type="button"  color="reset" fullWidth variant="contained" sx={{ maxWidth: '120px', color: "#FFFFFF" }} onClick={handleRejectedSubmit}>
