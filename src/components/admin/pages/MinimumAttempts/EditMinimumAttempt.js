...
const EditMinimumAttempt = () => {
    ...
    useEffect(()=>{
        if(id){
            ...
            setLoading(true);
            MinimumAttempt.getMinimumAttemptById(decrypt(id))
            .then((response)=>{

                setMinimumAttemptsFormValues({

                    attemptId: response.data.attemptId,
                    applicationScrutiny:response.data.applicationScrutiny,
                    applicationReview:response.data.applicationReview,
                    applicationAudit:response.data.applicationAudit,
                    esignApplicationReview:response.data.esignApplicationReview,
                    annualAuditReview:response.data.annualAuditReview,
    ...
    const handleFormSubmit = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            if(captchaInput === captchaText){
                ...
                //Save
                MinimumAttempt.updateMinimumAttempt(minimumAttemptsFormValues)
                .then((response)=>{

                    showAlert({
                        messageTitle: 'Update Minimum Attempts',
    ...
    return (
        <>
            ...
            <LoaderProgress open={isLoading} />
                ...
                <FormWrapper headingText="Update Minimum Attempt">
    ...
    <Grid container  spacing={2} direction="column">
        ...
        <Grid item xs={12} sm>
            <InputLabel shrink={false} htmlFor={"esignApplicationReview"}>
                <Typography variant='body1'>esign Application Review Attempt*</Typography>
            </InputLabel>
            <TextField
...
export default EditMinimumAttempt;
