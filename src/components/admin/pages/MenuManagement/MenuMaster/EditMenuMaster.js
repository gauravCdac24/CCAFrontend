...
const errorMsg = {
    ...
    menuOrder:{
        blank: "Please select menu order.",
    },
...
const EditMenuMaster = () => {
    ...
    useEffect(()=>{
        ...
        if(id){
            ...
            getAllMenuOrderList();
            getAllRoleMaster();
            MenuMasterService.getMenuById(decrypt(id))
            .then((response)=>{
    ...
    const validateForm = () => {
        ...
        if (!menuFormValues.menuOrder) {
            errors.menuOrder = errorMsg.menuOrder.blank;
        }
    ...
    const getAllMenuOrderList = () => {
        setLoading(true);

        MenuMasterService.getAllOrderList()
            .then((response) => {
                
                setMenuOrderList(response.data);
            })
            .catch((err) => {
                showAlert({
                    messageTitle: 'Error',
                    messageContent: 'Error fetching order list. Please try again later.',
                    confirmText: 'OK',
                });
            })
            .finally(() => {
    ...
    const handleFormSubmit = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            if(captchaInput === captchaText){
                setLoading(true);
                //Save
                MenuMasterService.updateMenu(menuFormValues)
                .then((response)=>{

                    showAlert({
                        messageTitle: 'Update Menu',
    ...
    return (
        <>
            ...
            <LoaderProgress open={isLoading} />
                ...
                <FormWrapper headingText="Update Menu">
                    ...
                    <Box component="form" noValidate sx={{ mt: 2, p: 2}} onSubmit={handleFormSubmit} >
                        ...
                        <Grid container  spacing={2} direction="column">
                            ...
                            <Grid item >
                                ...
                                <InputLabel shrink={false} htmlFor={"menuOrder"}>
                                    <Typography variant='body1' sx={{color: "#000000"}}>Menu Order*</Typography>
                                </InputLabel>
                                ...
                                <FormControl variant="outlined" size="small" sx={{  mt: 1, width: '100%' }}>
                                    ...
                                    <Select
                                        ...
                                        onChange={handleInputChange}
                                        displayEmpty
                                        value={menuFormValues.menuOrder || ''}
                                        name="menuOrder"
                                        error={
                                            Boolean(formErrors.menuOrder)
                                        }
                                        

                                        >

                                            <MenuItem disabled value={menuFormValues.menuOrder}>
                                                Menu Order
                                            </MenuItem>


                                            {
                                                menuOrderList.map((item, index)=>(
                                    ...
                                    {formErrors.menuOrder && (
                                    <FormHelperText error sx={{ml:0}}>{formErrors.menuOrder}</FormHelperText>
                                )}
                                </FormControl>
                                </Grid>
