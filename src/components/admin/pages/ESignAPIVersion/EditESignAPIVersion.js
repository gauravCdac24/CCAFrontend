...
const EditEsignAPIVersion = () => {
    ...
    const getDocument = async (id, filename) => {
        try {
    ...
    const handleGetDocument = async (id, filename) => {
        const fileDoc = await getDocument(id, filename);  
        console.log(fileDoc);  
        setVerFile(fileDoc);
    };
    ...
    useEffect(()=>{
        if(id){
            ...
            setLoading(true);
            ESignAPIVersionMasterService.getAPIVersionByID(decrypt(id))
            .then((response)=>{

                setAPIVerForm({
                    esignApiVerId: response.data.esignApiVerId?response.data.esignApiVerId:'',
                    esignApiSpecId: response.data.esignApiSpecId.esignApiSpecId?response.data.esignApiSpecId.esignApiSpecId:'',
                    apiVersion: response.data.apiVersion?response.data.apiVersion:'',
                    releaseDate: response.data.releaseDate?response.data.releaseDate:'',
                    espReadinessBy: response.data.espReadinessBy?response.data.espReadinessBy:'',
                    deprecationDate: response.data.deprecationDate?response.data.deprecationDate:'',
                    created: response.data.created?response.data.created:'',
                    updated: response.data.updated?response.data.updated:'',
                    createdBy: response.data.createdBy?response.data.createdBy:'',
                    updatedBy: response.data.updatedBy?response.data.updatedBy:'',
        
                })  

                handleGetDocument(decrypt(id), response.data.fileName);
            ...
            .catch((err)=>{
                navigate(`${rolePath}/apiversion`, { replace: true })
            })
            .finally(()=>{
                setLoading(false);
            })
    ...
    const validateFile = (errors) => {
        ...
        if (verfile) {
            ...
            if(verfile.type !== "application/pdf"){
                errors.file = errorMsg.file.format;
            }
    ...
    const handleFormSubmit = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            if(captchaInput === captchaText){
                ...
                //Save
                ESignAPIVersionMasterService.updateAPIVersion(apiVerForm, verfile)
                .then((response)=>{
...
export default EditEsignAPIVersion;
