...
import { Box, Typography, IconButton, TextField, Button, Collapse, Table, TableHead, TableBody, TableRow, TableCell, TableContainer, Radio, RadioGroup, FormControlLabel, FormControl, FormLabel } from '@mui/material';
...
const CASelfAssessment = () => {
  ...
  const getCASoftwareDetailsDetails = async () =>{
      ...
      setLoading(true);
      try{
          const response = await AnnexureService.getSelfAssessmentDetails();
  ...
  const handleSubmit = async() => {
    if (validate()) {
      ...
      try {
        ...
        const response = await AnnexureService.addSelfAssessmentDetails(caData);
