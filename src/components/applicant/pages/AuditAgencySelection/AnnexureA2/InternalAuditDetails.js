...
const InternalAuditDetails = () => {
    const [data, setData] = useState([
            { id: 1, inAuditDetailsId: '', description: 'Date of Last Internal Audit', fromDate: '', toDate: '', observations: '', auditorDetails: '' },
            { id: 2, inAuditDetailsId: '', description: 'Date of next Internal Audit', fromDate: '', toDate: '', observations: '', auditorDetails: '' },
        ]
    );
    ...
    const getInternalAuditDetails = async () =>{

        setLoading(true);
        try{
            const response = await AnnexureService.getInternalAuditDetails();
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            try{
                ...
                const response = await AnnexureService.addInternalAuditDetails(data);
                setLoading(false);

                showAlert({
                    messageTitle: 'Add Internal Audit Details',
                                    ...
                                    <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>
                                        ...
                                        <TableCell sx={{ border: 0.5, borderColor: 'grey.500' }}>
                                            <TextField
                                                ...
                                                onChange={(e) =>
                                                    handleChange(row.id, 'auditorDetails', e.target.value)
                                                }
                                                fullWidth
