...
const CPSDocumentDetails = ({ handleNext, handleBack }) => {
    ...
    const getAllDocType = () => {
        ...
        DocumentService.getAllAciveDocumentName()
            .then((response) => {
    ...
    const [fileNames, setFileNames] = useState(Array(docTypeList.length).fill(''));
    const [fileErrors, setFileErrors] = useState(Array(docTypeList.length).fill(''));
  ...
  const handleFileChange6 = (index) => (event) => {
    ...
    if (!file) {
        setFileErrors(prev => {
            const errs = [...prev];
            errs[index] = 'Please select a file';
            return errs;
        });
        return;
    }
    ...
    setBasicDetailsFormValues(prev => {
        const updatedFiles = [...prev.additionalsFile];
        updatedFiles[index] = {
            file,
            fileName: file.name,
            DocumentName: docTypeList[index]?.documentName || '',
            appDocId: '',
        };
        return { ...prev, additionalsFile: updatedFiles };
    });
        ...
        if (userName) {
            ...
            ApplicationForm.getApplicationForm6ByUsername(userName)
                .then((response) => {
                   ...
                   const docTypeNames = docTypeList?.map(d => d.documentName);
                  
const specificDocs = documents.filter(doc => docTypeNames.includes(doc.documentTitle))
  .map(doc => ({
    ...
    file: null
                    ...
                    const otherDocuments = documents.filter(doc =>
                        ...
                        !docTypeNames.includes(doc.documentTitle)
                    ).map(doc => ({
                        appDocId: doc.appDocId,
    ...
    const handleFormSubmit = (e) => {
        ...
        docTypeList.forEach((item, index) => {
            if (!fileNames[index]) {
                newErrors[index] = 'Please select a file';
                isValid = false;
        ...
        if (Object.keys(validationErrors).length === 0 && isValid) {
            ...
            basicDetailsFormValues.additionalFile.forEach((fileObject, index) => {
                ...
                formData.append(`additionalFile[${index}].appDocId`, fileObject.appDocId);
            });
            ...
            basicDetailsFormValues.additionalsFile.forEach((fileObject, index) => {
                ...
                formData.append(`additionalsFile[${index}].appDocId`, fileObject.appDocId);
            });
                        ...
                        <Grid item xs={12} sm={3}>
                            <InputLabel shrink={false} htmlFor={`pdffile-${index}`}>
                                <Typography variant='body1' sx={{ mt: 5 }}>
                                    {index + 1}. {item.documentName} Document
                                </Typography>
                            </InputLabel>
                        </Grid>
                        <Grid item xs={12} sm={7}>
                            <InputLabel shrink={false} htmlFor={`pdffile-${index}`}>
                                <Typography variant='body1'>
                                    Upload {item.documentName} Document (Only PDF and Max allowed size 5 MB)*
                                </Typography>
                            </InputLabel>
