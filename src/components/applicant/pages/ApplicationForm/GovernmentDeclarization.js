...
import GovernmentPreviewModal from './GovernmentPreviewModal';

const GovernmentDeclarization = ({ handlePreview, handleNext, handleBack, data }) => {
  ...
  const getAllUndertaking = () => {
    ...
    Undertaking.getAllUndertakingList()
      .then((response) => {
        ...
        response.data
          .filter(item => item.appTypeMasterId.appTypeMasterId === 4)
          .forEach((item) => {
...
const Yes = () => {
  ...
  const preViewData = () => {
    showAlert({
      ...
      messageContent: (
        <>
          <GovernmentPreviewModal checkedState={checkedState} data={data} />
        </>
      ),
      confirmText: 'Ok',
      enableHeaderCloseBtn: true,
  ...
  return (
    <Box component="form" noValidate sx={{ mt: 2, p: 2 }} onSubmit={handleFormSubmit}>
      <Grid container spacing={2}>
        ...
        {allSubServiceList
          .filter(item => item.appTypeMasterId.appTypeMasterId === 4)  // Filter for appTypeMasterId = 1
...
export default GovernmentDeclarization;
