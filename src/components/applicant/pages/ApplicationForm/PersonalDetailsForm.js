...
const isFieldEnabled = (fieldName) => {
    ...
    return isEnabled;
      ...
      const mapEmailRecords = (emails) => {
        if (!Array.isArray(emails) || emails.length === 0) {
            return [{ emailId: '', indivEmailId: '' }];
        }

        return emails.map((entry) => ({
            emailId: entry?.emailId || entry?.email || '',
            indivEmailId: entry?.indivEmailId ?? '',
        }));
      };

      const normalizeEmailsForSubmit = (emails = []) => {
        const seen = new Set();

        return emails
            .map((entry) => ({
                emailId: (entry?.emailId || entry?.email || '').trim(),
                indivEmailId: entry?.indivEmailId ?? '',
            }))
            .filter((entry) => {
                const key = entry.emailId.toLowerCase();
                if (!key || seen.has(key)) {
                    return false;
                }
                seen.add(key);
      ...
      useEffect(() => {
        ...
        const loadPersonalDetails = async () => {
            try {
                const response = await ApplicationForm.getApplicationFormByUsername(userName);
                if (isCancelled) {
                    return;
                }

                const { application, emails } = response.data;
                const isKnownByOtherName = application.firstName2 !== '';

                setBasicDetailsFormValues((prevState) => ({
                    ...prevState,
                    emailId: mapEmailRecords(emails),
                    salutation: application.salutation1 || '',
                    firstName: application.firstName1 || '',
                    middleName: application.middleName1 || '',
                    lastName: application.lastName1 || '',
                    fathersSalutation: application.fsalutation || '',
                    fatherFirstName: application.ffirstName || '',
                    fatherMiddleName: application.fmiddleName || '',
                    fatherLastName: application.flastName || '',
                    knownByOtherName: isKnownByOtherName,
                    otherFirstName: application.firstName2 || '',
                    otherMiddleName: application.middleName2 || '',
                    otherLastName: application.lastName2 || '',
                    dob: application.dob ? new Date(application.dob) : null,
                    gender: application.gender || '',
                    nationality: application.nationality || '',
                    indivAppId: application.indivApplicationId,
                    intentAppId: application.intentAppId.intentAppId,
                    userName: application.intentAppId.userName || userName,
                }));
                ...
                try {
                    ...
                    const intentEmail = intentResponse.data?.emailId || '';
                    setBasicDetailsFormValues((prevState) => ({
                        ...prevState,
                        emailId: intentEmail
                            ? [{ emailId: intentEmail, indivEmailId: '' }]
                            : [{ emailId: '', indivEmailId: '' }],
                        userName,
                    }));
                } catch (error) {
...
console.log(basicDetailsFormValues.intentAppId)
    ...
    const handleAddssField = () => {
        const emailId = [...basicDetailsFormValues.emailId, { emailId: '', indivEmailId: '' }];
        setBasicDetailsFormValues({ ...basicDetailsFormValues, emailId });
    };
    ...
    const handleFormSubmit = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            const payload = {
                ...
                emailId: normalizeEmailsForSubmit(basicDetailsFormValues.emailId),
            };
