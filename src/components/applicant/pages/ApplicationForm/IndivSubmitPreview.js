...
const IndivSubmitPreview = forwardRef(({allUndertakingsList}, ref)  => {
...
const handleSubmit = () => {
  showAlert({
    ...
    maxWidth: 'sm',
    fullWidth: true,
    onConfirm: () => continues(), // Call main submit from Declarization
...
console.log("allUndertakingsList---->",allUndertakingsList)

  const continues = async () => {
    setLoading(true);

    ApplicationForm.addApplicationForm(allUndertakingsList)
      .then((response) => {
        console.log(JSON.stringify(response));
        if (response) {
          showAlert({
            messageTitle: 'Success',
            messageContent: 'Application Form successfully submitted',
            confirmText: 'OK',
            disableOutsideKeyDown: true,
...
export default IndivSubmitPreview;